...
/* ****************************************************************
* Classe: ThreadTremAmarelo
* Funcao: Representar o Trem Amarelo e seus comportamentos na tela.
...
public class ThreadTremAmarelo extends Thread {
  // Atributos
  private ImageView imageViewTremAmarelo;
  ...
  private int vezDoTremAmarelo = 0;// vez de executar a regiao critica (estrita alternancia)
  private int idTremAmarelo = 0;// posicao no vetor de interesse referente ao trem (solucao de peterson)

  // Construtor da classe
  public ThreadTremAmarelo(TelaPrincipal telaPrincipal, ImageView imageViewTremAmarelo, Image imagemNoLadoEsquerdo,
      Image imagemNoLadoDireito) {
    super("ThreadTremAmarelo");// define o nome da thread
    this.telaPrincipal = telaPrincipal;
    this.imageViewTremAmarelo = imageViewTremAmarelo;
  ...
  public void run() {// acoes a serem tomadas ao iniciar a thread
    while (!isInterrompida) {// looping enquanto a thread nao foi interrompida
      switch (telaPrincipal.getSolucaoEscolhida()) {// comportamento de acordo coma solucao escolhida
        ...
        case (2): {// ESCOLHEU ESTRITA ALTERNANCIA
          if (escolheuLadoEsquerdo()) {// se a posicao para o trem escolhida for o lado Esq
            ...
            andarDaEsquerdaParaDireitaNa_RC_2();// anda na RC_2
            telaPrincipal.sinalizarVezDoProximoExecutar_RC_2();// informa que eh a vez do proximo passar na RC_1
            // inicio da regiao critica 2
            ...
            break;
          } // fim do bloco if-else (estrita alternancia)
        ...
        case (3): {// ESCOLHEU SOLUCAO DE PETERSON
          if (escolheuLadoEsquerdo()) {// se a posicao para o trem escolhida for o lado Esq
            ...
            // inicio da regiao critica 1
            telaPrincipal.ENTRAR_RC_1(idTremAmarelo);// sinaliza interesse e verifica o estado da RC_1
            andarDaEsquerdaParaDireitaNa_RC_1();// anda na RC_1
            telaPrincipal.DEIXAR_RC_1(idTremAmarelo);// informa que a RC_1 esta vazia
            ...
            // inicio da regiao critica 2
            telaPrincipal.ENTRAR_RC_2(idTremAmarelo);// sinaliza interesse e verifica o estado da RC_2
            andarDaEsquerdaParaDireitaNa_RC_2();// anda na RC_2
            telaPrincipal.DEIXAR_RC_2(idTremAmarelo);// informa que a RC_2 esta vazia
            // inicio da regiao critica 2
  ...
  public synchronized void verificarVezDeExecutar_RC_1() {
    while (telaPrincipal.vezDeExecutar_RC_1() != vezDoTremAmarelo) {// espera ate a ser a vez do trem percorrer a RC_1
      try {
        Thread.sleep(1);
      } catch (InterruptedException exc) {// se a thread foi interrompida
        interromper();// quebra o laco do metodo run()
    ...
    notifyAll();
  }// fim do metodo verificarVezDeExecutar_RC_1

  public synchronized void verificarVezDeExecutar_RC_2() {
    while (telaPrincipal.vezDeExecutar_RC_2() != vezDoTremAmarelo) {// espera ate a ser a vez do trem percorrer a RC_2
      try {
        Thread.sleep(1);
      } catch (InterruptedException exc) {// se a thread foi interrompida
        interromper();// quebra o laco do metodo run()
  ...
  // POSICIONAMENTO E MOVIMENTACAO
  public void mudarParaLadoEsquerdoDaTela() {
    // muda a posicao de partida do trem
    posicao_X_Atual = 0;
    posicao_Y_Atual = 200;
    // muda o trem para o lado esquerdo dos trilhos
    Platform.runLater(() -> imageViewTremAmarelo.setImage(imagemNoLadoEsquerdo));// troca a imagem exibida
    Platform.runLater(() -> imageViewTremAmarelo.setLayoutX(posicao_X_Atual));// muda a posicao da imagem
    Platform.runLater(() -> imageViewTremAmarelo.setLayoutY(posicao_Y_Atual));// muda a posicao da imagem
  ...
  public void mudarParaLadoDireitoDaTela() {// muda o trem para o lado direito dos trilhos
    ...
    Platform.runLater(() -> imageViewTremAmarelo.setImage(imagemNoLadoDireito));// troca a imagem exibida
    Platform.runLater(() -> imageViewTremAmarelo.setLayoutX(posicao_X_Atual));// muda posicao da imagem
    Platform.runLater(() -> imageViewTremAmarelo.setLayoutY(posicao_Y_Atual));// muda posicao da imagem
  ...
  public boolean escolheuLadoEsquerdo() {
    if (telaPrincipal.getPosicaoDosTrensEscolhida() == 1 ||
        telaPrincipal.getPosicaoDosTrensEscolhida() == 2) {
  ...
  public void subir(int novaPosY) {// novaPosY eh o nova coordenada do trem na tela
    while (posicao_Y_Atual != novaPosY) {// anda no eixo Y decrementando o seu valor ate atingir a nova posicao
      ...
      Platform.runLater(() -> imageViewTremAmarelo.setLayoutY(posicao_Y_Atual));// atualiza a pos do trem na tela
    } // fim do while
  }// fim do metodo subir
  ...
  public void moverParaDireita(int novaPosX) {// novaPosX eh o nova coordenada do trem na tela
    while (posicao_X_Atual != novaPosX) {// anda no eixo X incrementando o seu valor ate atingir a nova posicao
      ...
      Platform.runLater(() -> imageViewTremAmarelo.setLayoutX(posicao_X_Atual));// atualiza a pos do trem na tela
    } // fim do while
  ...
  // COMPORTAMENTO NAS REGIOES DOS TRILHOS TREM ANDANDO (ESQUERDA -> DIREITA)
  ...
  // COMPORTAMENTO NAS REGIOES DOS TRILHOS TREM ANDANDO (DIREITA -> ESQUERDA)
...
}// fim da classe ThreadTremAmarelo